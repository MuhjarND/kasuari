...
$sql = "SELECT hatibinwasbid.* FROM hatibinwasbid ORDER BY id DESC";
$result = $conn->query($sql);
$no = 1;

if ($result === false) {
    error_log('KASUARI hatibinwasbid load failed: '.$conn->error);
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        $periode = htmlspecialchars((string) ($row['periode'] ?? ''), ENT_QUOTES, 'UTF-8');
